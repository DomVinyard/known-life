...
//
// The GitHub-token half of the old `redeem`, decoupled from cf-drop. The
// device-flow OAuth bridge caches the user's GitHub access token at
// gh:tok:<login> (elevated-scope grant); this hands it back to the JWT-bound
// setup process so it can create the repo (POST /user/repos) and register the
// lifekey (POST /user/keys). The Cloudflare half is now the cf-oauth broker
// (start → status → token), so the setup flow no longer needs cf-drop/redeem.
// Same security model as redeem: the GitHub token is returned only to a caller
